...
public class PlcModbusCommunicator : IPlcClient
{
    ...
    public async Task ConnectAsync()
    {
        ...
        try
        {
            ...
            if (await Task.WhenAny(connectTask, Task.Delay(ConnectTimeoutMs)).ConfigureAwait(false) != connectTask) // 5秒连接超时
            {
    ...
    public void Connect() 
    {
        ...
        try
        {
            ...
            // --- 连接成功后的操作 ---
            _modbusMaster = ModbusIpMaster.CreateIp(_tcpClient);
            _modbusMaster.Transport.ReadTimeout = TransportReadTimeoutMs; // 仍然可以设置读写超时
            _modbusMaster.Transport.WriteTimeout = TransportWriteTimeoutMs;
    ...
    public void GetData()
    {
        ...
        foreach (ushort address in LeftFloatAddresses)
        {
            LeftValue.Add(ReadFloat(address));
        }

        foreach (ushort address in LeftCoilAddresses)
        {
            LeftValue.Add(ReadSingleCoil(address));
        }

        LeftValue.Add(ReadFloat(LeftTailFloatAddress));

        foreach (ushort address in RightFloatAddresses)
        {
            RightValue.Add(ReadFloat(address));
        }

        foreach (ushort address in RightCoilAddresses)
        {
            RightValue.Add(ReadSingleCoil(address));
        }
    }
